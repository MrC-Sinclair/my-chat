...
- [ ] 1.1 修改 `docker-compose.yml`，将 `postgres` 和 `test-postgres` 服务镜像切换为 `pgvector/pgvector:pg17`；在 README 或迁移说明中记录需执行 `docker compose down -v` 删除旧 volume 后重建
...
- [ ] 1.3 修改 `server/db/schema.ts`：导入 Drizzle 原生 `vector`（from `drizzle-orm/pg-core`），新增 `memoryVectors` 表（字段：`id` 主键、`message_id` 外键→messages.id 级联删除、`session_id` 外键→sessions.id 级联删除、`content` text NOT NULL、`embedding` 使用 `vector({ dimensions: 1024 })` NOT NULL、`role` text NOT NULL、`created_at` timestamp NOT NULL defaultNow()、`archived_at` timestamp NOT NULL defaultNow()）
- [ ] 1.4 在 pgTable 第二个参数回调中创建 HNSW 向量索引（`index('memory_embedding_idx').using('hnsw', table.embedding.op('vector_cosine_ops'))`），索引参数 `m` / `ef_construction` 通过环境变量 `HNSW_M` / `HNSW_EF_CONSTRUCTION` 覆盖（默认 16 / 64）
- [ ] 1.5 运行 `pnpm db:push` 同步 schema 到开发数据库和测试数据库，验证表 + 索引创建成功
- [ ] 1.6 同步更新 `docs/db-schema.md` 记录 `memory_vectors` 表结构（含 `archived_at` 字段）

## 2. 配置扩展

- [ ] 2.1 在 `nuxt.config.ts` runtimeConfig 新增非 public 字段：`embeddingModel`、`rerankerModel`、`memoryImportanceModel`、`hnswM`、`hnswEfConstruction`
- [ ] 2.2 在 `.env.example` 中新增对应可选环境变量说明（默认值：embedding=`BAAI/bge-m3`、reranker=`BAAI/bge-reranker-v2-m3`、importance=`Qwen/Qwen3.5-4B`、HNSW m=16 ef_construction=64）
...
- [ ] 5.2 实现 embedding 检索逻辑：query 转向量 → 使用 Drizzle 原生 `cosineDistance()` + 余弦距离（`<=>` 算子）排序取 top-20
...
- [ ] 6.1 新增 `server/utils/memory-archive.ts`，实现重要度判断逻辑：默认使用 `Qwen/Qwen3.5-4B`，直接通过 `fetch` 调用硅基流动 `/v1/chat/completions`（非流式），请求体传 `enable_thinking: false`、`temperature: 0.1`、`max_tokens: 4096`、`stream: false`，输出严格 JSON 数组判断每条消息重要度
- [ ] 6.2 实现 30 秒超时（AbortController），超时则该次归档整体失败（记录日志）
- [ ] 6.3 实现 JSON 解析容错：若 LLM 返回含 `<think>` 标签或格式错误，整体降级为不入库（避免污染记忆库）
- [ ] 6.4 实现入库逻辑：对 LLM 判断为重要的消息调用 embedding 服务 → 写入 `memory_vectors` 表（同时记录 `archived_at`）
...
- [ ] 8.3 在 `onFinish` 回调中加入服务端归档兜底：若 `lastSessionId` 存在且不等于当前 `sessionId`，则异步触发该会话归档（不阻塞对话流返回）；不引入全局会话缓存，保持服务端无状态
- [ ] 8.4 验证 `maxSteps` 动态计算正确（`hasActiveTools` 包含 recall-memory）
- [ ] 8.5 运行 `pnpm lint` + `pnpm typecheck` 验证

## 9. 前端触发归档

- [ ] 9.1 在 `composables/useChatSession.ts` 的 `switchSession()` 和 `createNewSession()` 方法中，对切换前的 `currentSessionId`（上一个会话）异步调用 `POST /api/sessions/:id/archive-memory`
- [ ] 9.2 在发起 `/api/chat` 请求时，将上一个会话 ID 作为 `lastSessionId` 传入请求 body（通过 computed body 确保响应式更新）
...
- [ ] 11.4 新增 `tests/unit/memory-archive.test.ts`，测试归档逻辑（enable_thinking:false LLM 调用、重要度 JSON 解析、思考标签容错、消息级重复守卫、进程内并发锁、失败容错超时、角色/敏感信息过滤、archived_at 写入）
