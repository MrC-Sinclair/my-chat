...
- 调整 `docker-compose.yml`，使用 `pgvector/pgvector:pg18` 镜像以支持向量扩展（与当前 `postgres:18-alpine` 同为 PG 18，无需 `docker compose down -v`，数据卷兼容）
...
| 层级 | 影响 |
| --- | --- |
| 基础设施 | `docker-compose.yml` 的 PostgreSQL 镜像切换为 `pgvector/pgvector:pg18`（与当前 `postgres:18-alpine` 同为 PG 18，无需 `docker compose down -v`，数据卷兼容）；执行 `docker compose up -d` 重建容器即可 |
