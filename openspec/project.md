...
基于 Nuxt 3 + Vercel AI SDK 的 AI 对话应用，支持 Markdown + LaTeX 公式安全渲染、图片对话（多模态），内置工具调用（天气、搜索、OCR）。适配平板和手机屏幕。

> **架构演进方向**：本项目目标是从「带工具的对话应用」演进为「Agent 架构」。当前处于轻量 Agent 阶段（LLM 在 `streamText` 的 `stopWhen` 循环内自主决策工具调用），后续逐步引入规划层、记忆系统、状态管理与检查点恢复。新增功能必须按 Agent 设计原则实现，不得回退为纯对话或纯工作流模式。详见 `AGENTS.md` 的「Agent 架构设计规范」章节。

## 架构定位与演进路线

| 阶段 | 形态 | 控制流归属 | 当前状态 |
| --- | --- | --- | --- |
| 阶段 0 | 纯对话应用 | 单次 `streamText` 调用，无工具 | 已超越 |
| 阶段 1 | 带工具的对话 Agent | `streamText` + 工具注册 + `stopWhen` 多步循环，LLM 自主决策 | ✅ 当前 |
| 阶段 2 | Agentic RAG | 阶段 1 + 检索作为工具暴露给 LLM 自主调用 | 规划中 |
| 阶段 3 | 长程 Agent | 阶段 2 + 任务规划层 + 记忆系统 + 工件管理 + 检查点恢复 | 远期 |

**核心判定标准**：控制流由谁决定——代码预编排（Workflow）还是 LLM 自主决策（Agent）。本项目所有新功能默认走 Agent 路径（LLM 自主决策），仅在确有必要的护栏场景（如安全、合规）使用 Workflow 式预编排，并须在 openspec design 文档中说明理由。
