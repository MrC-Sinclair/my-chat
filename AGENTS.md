...
| `IMGBB_API_KEY`     | 否     | ImgBB 图床 API Key（已硬编码在 `server/utils/imgbb.ts`，可覆盖） | —                       |
...
- **图片对话使用 ImgBB 图床**：硅基流动不支持 base64 图片，需先上传到 ImgBB 获取公网 URL。API Key 硬编码在 [imgbb.ts](file:///d:/code/codeWork/my-chat/server/utils/imgbb.ts)，免费注册 https://api.imgbb.com/ 获取
