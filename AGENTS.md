...
- **关键操作必须验证**：启动数据库后用 `docker compose ps` 确认容器 running；启动开发服务器后检查终端输出无 ERROR；安装依赖后检查退出码为 0；数据库 Schema 同步后检查退出码为 0
- **交叉验证原则**：当工具返回的结果会影响后续决策时，必须用另一种工具交叉验证。例如：文件搜索工具说文件不存在时，用 `Test-Path` 再确认；命令输出看似成功时，检查退出码是否为 0
- **禁止用 Glob 检查文件/目录是否存在**：Glob 对隐藏文件（以 `.` 开头，如 `.env`）和目录（如 `node_modules`）的匹配不可靠，会返回空结果导致误判。文件存在性检查用 `Test-Path`，目录内容列表用 `LS`，Glob 仅用于按模式批量搜索文件名
