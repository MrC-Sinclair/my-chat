...
- **修改代码后必须运行** **`pnpm lint`** **检查 lint 错误**。特别是模板中的 HTML 结构变更（如添加/删除标签），每次编辑后都要手动校验对应的起始/闭合标签是否完整

## AI Agent 执行纪律

以下规则适用于所有 AI 编程助手（Trae、Cursor、Claude Code 等），防止 Agent 跳步或误判：
...
- **多方案确认模式（强制）**：遇到多种技术实现路径、需求描述有模糊空间、或需分步骤执行时，必须使用 `AskUserQuestion` 工具弹框列出 3-4 个清晰选项（注明推荐理由），等待用户勾选回复后方可继续，严禁手写 checkbox 纯文本或直接编写完整代码结束对话
...
| 层级     | 测试类型         | 必须程度 | 触发时机       | 命令                                                      |
| -------- | ---------------- | -------- | -------------- | --------------------------------------------------------- |
| 静态分析 | 类型检查         | 🔴 每次  | 提交前         | `pnpm typecheck`                                          |
| 静态分析 | ESLint           | 🔴 每次  | 提交前         | `pnpm lint`                                               |
| 单元测试 | Vitest           | � 每次   | 提交前         | `pnpm test:unit`                                          |
| E2E 测试 | Playwright       | 🟡 核心  | 修改渲染逻辑后 | `pnpm test:e2e`                                           |
| 视觉回归 | 截图对比         | 🟢 发版  | 发版前 / CI    | `npx playwright test tests/e2e/visual-regression.spec.ts` |
| 跨浏览器 | Firefox / WebKit | 🟢 发版  | 发版前 / CI    | `npx playwright test --project=firefox`                   |
| 构建验证 | 生产构建         | 🟡 核心  | 发版前         | `pnpm build`                                              |
